...
class UserMapper
{
    ...
    {
        try {
            $sql = 'SELECT `name` FROM `users_accs` WHERE `id` = :id';
    ...
    {
        try {
            $sql = 'SELECT `id`, `name` FROM `users_accs` WHERE `name` = :name';
            $stmt = $this->pdo->prepare($sql);
    ...
    {
        try {
            $sql = 'SELECT `id` FROM `users_accs` WHERE `name` = :name';
            $stmt = $this->pdo->prepare($sql);
    ...
    {
        try {
            $sql = 'INSERT INTO `users_accs`(`name`, `hash`) VALUES (:name, :hash)';
            $stmt = $this->pdo->prepare($sql);
            $stmt->bindParam(':name', $name);
    ...
    {
        try {
            $sql = 'SELECT `hash` FROM `users_accs` WHERE `id` = :id';
            $stmt = $this->pdo->prepare($sql);
    ...
    {
        try {
            $sql = 'SELECT `hash` FROM `users_accs` WHERE `name` = :name';
            $stmt = $this->pdo->prepare($sql);
            $stmt->bindParam(':name', $username);
            $result = $stmt->execute();
            if ($result !== false) {
    ...
    {
        try {
            $sql = 'UPDATE `users_accs` SET `hash` = :hash WHERE `name` = :name';
            $stmt = $this->pdo->prepare($sql);
            $stmt->bindParam(':name', $username);
    ...
    {
        try {
            $sql = 'DELETE FROM `users_accs` WHERE `id` = :id';
            $stmt = $this->pdo->prepare($sql);
            $stmt->bindParam(':id', $userID, \PDO::PARAM_INT);
            $result = $stmt->execute();
        } catch (\PDOException $e) {
